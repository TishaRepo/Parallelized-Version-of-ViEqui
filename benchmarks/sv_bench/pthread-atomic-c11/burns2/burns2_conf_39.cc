...
;
atomic_store_explicit( &_cc_x, 0, memory_order_relaxed);
rx = atomic_load_explicit( &_cc_x, memory_order_relaxed);
assert(rx <= 0);
atomic_store_explicit( &flag1, 0, memory_order_relaxed       );
        }
        return NULL;
...
rflag1 = atomic_load_explicit( &flag1, memory_order_relaxed);
