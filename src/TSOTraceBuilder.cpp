...
void TSOTraceBuilder::cancel_replay()
{
  if (!replay)
    return;
  replay = false;
  while (prefix_idx + 1 < int(prefix.len()))
    prefix.delete_last();
...
void TSOTraceBuilder::wut_string_add_node(std::vector<std::string> &lines, std::vector<int> &iid_map,
                                          unsigned line, Branch branch, WakeupTreeRef<Branch> node) const
{
  ...
  while (n.size())
  {
    ...
    if (l < lines.size())
      offset = std::max(offset, unsigned(lines[l].size()));
  }
  if (lines.size() < l + 1)
    lines.resize(l + 1, "");
  /* First node needs different padding, so we do it here */
  lines[line] += " ";
  while (lines[line].size() < offset)
    lines[line] += "-";
...
void TSOTraceBuilder::check_symev_vclock_equiv() const
{
  ...
  for (unsigned i = 0; i < prefix.len(); ++i)
  {
    ...
    const Event *prev = (e.iid.get_index() == 1 ? nullptr
                                                : &prefix[find_process_event(pid, e.iid.get_index() - 1)]);
    if (i == prefix.len() - 1 && errors.size() &&
        errors.back()->get_location() == IID<CPid>(threads[pid].cpid, e.iid.get_index()))
    {
    ...
    {
      bool iafterj = false;
      if (prefix[j].iid.get_pid() == pid || do_events_conflict(e, prefix[j]))
      {
        iafterj = true;
...
bool TSOTraceBuilder::atomic_store(const SymData &sd)
{
  if (conf.dpor_algorithm == Configuration::OBSERVERS)
  {
    if (!record_symbolic(SymEv::UnobsStore(sd)))
      return false;
  }
  else
  {
...
{
  return e.kind == SymEv::UNOBS_STORE || e.kind == SymEv::STORE || e.kind == SymEv::CMPXHG;
}
...
static SymAddrSize sym_get_last_write(const sym_ty &sym, SymAddr addr)
{
  ...
  {
    const SymEv &e = *(--it);
    if (symev_does_store(e) && e.addr().includes(addr))
      return e.addr();
  }
...
void TSOTraceBuilder::do_atomic_store(const SymData &sd)
{
  ...
  if (is_update)
  { // Add the clock of the store instruction
...
bool TSOTraceBuilder::compare_exchange(const SymData &sd, const SymData::block_type expected, bool success)
{
  if (success)
  {
    if (!record_symbolic(SymEv::CmpXhg(sd, expected)))
      return false;
    do_load(sd.get_ref());
    do_atomic_store(sd);
  }
  ...
  {
    if (!record_symbolic(SymEv::CmpXhgFail(sd, expected)))
      return false;
    do_load(sd.get_ref());
  }
  return true;
...
{
  if (!record_symbolic(SymEv::Fullmem()))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
  ...
  if (0 <= lu)
  {
    ...
    if (conf.dpor_algorithm == Configuration::OBSERVERS)
    {
      ...
      for (auto it = prefix[lu].sym.end();;)
      {
        ...
        --it;
        if ((it->kind == SymEv::STORE || it->kind == SymEv::CMPXHG) && it->addr() == lu_ml)
          break;
...
{
  if (!record_symbolic(SymEv::MLock(ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
{
  if (!record_symbolic(SymEv::MUnlock(ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
{
  if (!record_symbolic(SymEv::MInit(ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
{
  if (!record_symbolic(SymEv::MDelete(ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
{
  if (!record_symbolic(SymEv::CInit(ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
{
  if (!record_symbolic(SymEv::CSignal(ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
  ...
  {
    if (!register_alternatives(cond_var.waiters.size()))
      return false;
  }
...
{
  if (!record_symbolic(SymEv::CBrdcst(ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
bool TSOTraceBuilder::cond_wait(const SymAddrSize &cond_ml, const SymAddrSize &mutex_ml)
{
  ...
  if (!mutex_unlock(mutex_ml))
    return false;
  if (!record_symbolic(SymEv::CWait(cond_ml)))
    return false;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
bool TSOTraceBuilder::cond_awake(const SymAddrSize &cond_ml, const SymAddrSize &mutex_ml)
{
  ...
  if (!mutex_lock(mutex_ml))
    return false;
  if (!record_symbolic(SymEv::CAwake(cond_ml)))
    return false;
  if (dryrun)
  {
    return true;
...
int TSOTraceBuilder::cond_destroy(const SymAddrSize &ml)
{
  ...
  if (!record_symbolic(SymEv::CDelete(ml)))
    return err;
  if (dryrun)
  {
    assert(prefix_idx + 1 < int(prefix.len()));
...
{
  curev().may_conflict = true;
  if (!record_symbolic(SymEv::Nondet(alt_count)))
    return false;
  if (curbranch().alt == 0)
  {
...
template <typename T>
static inline void unordered_vector_delete(std::vector<T> &vec, std::size_t pos)
{
  assert(pos < vec.size());
  if (pos + 1 != vec.size())
...
void TSOTraceBuilder::obs_sleep_wake(struct obs_sleep &sleep, const Event &e) const
{
  if (conf.dpor_algorithm != Configuration::OBSERVERS)
  {
    if (e.wakeup.size())
...
{
  return e.kind == SymEv::LOAD || e.kind == SymEv::CMPXHG || e.kind == SymEv::CMPXHGFAIL || e.kind == SymEv::FULLMEM;
}
                          ...
                          const struct obs_sleep &sleep_const) const
{
  ...
  for (auto it = seq.cbegin(); state == obs_wake_res::CONTINUE && it != seq.cend(); ++it)
  {
...
template <class Iter>
static void rev_recompute_data(SymData &data, VecSet<SymAddr> &needed, Iter end, Iter begin)
{
...
void TSOTraceBuilder::recompute_cmpxhg_success(sym_ty &es, const std::vector<Branch> &v, int i) const
{
  ...
  for (auto vi = v.end(); vi != v.begin();)
  {
    ...
    for (auto ei = vb.sym.end(); ei != vb.sym.begin();)
    {
      ...
      case SymEv::STORE:
        ...
        abort();
      case SymEv::UNOBS_STORE:
        if (read_all ^ last_reads.intersects(VecSet<SymAddr>(e.addr().begin(), e.addr().end())))
        {
...
void TSOTraceBuilder::see_event_pairs(const VecSet<std::pair<int, int>> &seen_accesses)
{
...
void TSOTraceBuilder::add_noblock_race(int event)
{
  ...
  if (races.size())
  {
    ...
    if (prev.kind == Race::NONBLOCK && prev.first_event == event && prev.second_event == prefix_idx)
      return;
  }
...
void TSOTraceBuilder::add_observed_race(int first, int second)
{
  ...
  {
    const Race &prev = races.back();
    if (prev.kind == Race::OBSERVED && prev.first_event == first && prev.second_event == second && prev.witness_event == prefix_idx)
      return;
  }
...
void TSOTraceBuilder::add_happens_after_thread(unsigned second, IPid thread)
{
  ...
  if (threads[thread].event_indices.empty())
    return;
...
void TSOTraceBuilder::compute_vclocks()
{
  ...
  for (unsigned i = 0; i < prefix.len(); i++)
  {
    ...
    auto fill = frontier_filter(first_pair, end,
                                [this](const Race &f, const Race &s) {
                                  /* A virtual event does not contribute to the vclock and cannot
         * subsume races. */
                                  if (s.kind == Race::LOCK_FAIL)
                                    return false;
                                  /* Also filter out observed races with nonfirst witness */
                                  if (f.kind == Race::OBSERVED && s.kind == Race::OBSERVED && f.first_event == s.first_event && f.second_event == s.second_event)
                                  {
    ...
    for (auto it = first_pair; it != fill; ++it)
    {
      if (it->kind == Race::LOCK_SUC)
      {
        assert(prefix[it->first_event].clock.leq(prefix[it->unlock_event].clock));
        prefix[i].clock += prefix[it->unlock_event].clock;
      }
...
bool TSOTraceBuilder::do_events_conflict(const Event &fst, const Event &snd) const
{
...
bool TSOTraceBuilder::do_symevs_conflict(IPid fst_pid, const SymEv &fst,
                                         IPid snd_pid, const SymEv &snd) const
{
  if (fst.kind == SymEv::NONDET || snd.kind == SymEv::NONDET)
    return false;
  if (fst.kind == SymEv::FULLMEM || snd.kind == SymEv::FULLMEM)
    return true;
  if (symev_is_load(fst) && symev_is_load(snd))
    return false;
  if (symev_is_unobs_store(fst) && symev_is_unobs_store(snd) && fst.addr() == snd.addr())
    return false;
...
bool TSOTraceBuilder::do_events_conflict(IPid fst_pid, const sym_ty &fst,
                                         IPid snd_pid, const sym_ty &snd) const
{
  ...
  {
    if (symev_has_pid(fe) && fe.num() == (snd_pid / 2))
      return true;
  ...
  for (const SymEv &se : snd)
  {
    if (symev_has_pid(se) && se.num() == (fst_pid / 2))
      return true;
  }
  return false;
}

bool TSOTraceBuilder::is_observed_conflict(const Event &fst, const Event &snd, const Event &thd) const
{
...
bool TSOTraceBuilder::is_observed_conflict(IPid fst_pid, const sym_ty &fst,
                                           ...
                                           IPid thd_pid, const sym_ty &thd) const
{
  ...
  for (const SymEv &fe : fst)
  {
    if (fe.kind != SymEv::UNOBS_STORE)
      continue;
    for (const SymEv &se : snd)
    {
      if (se.kind != SymEv::STORE || !fe.addr().overlaps(se.addr()))
        continue;
...
bool TSOTraceBuilder::is_observed_conflict(IPid fst_pid, const SymEv &fst,
...
void TSOTraceBuilder::race_detect(const Race &race, const struct obs_sleep &isleep)
{
  if (conf.dpor_algorithm != Configuration::SOURCE)
  {
  ...
  for (int k = i + 1; k <= j; ++k)
  {
    ...
    if (p < int(candidates.size()) && 0 <= candidates[p])
      continue;
    const Event *pevent;
    int psize = 1;
...
void TSOTraceBuilder::race_detect_optimal(const Race &race, const struct obs_sleep &isleep)
{
  ...
  for (int k = i + 1; k < int(prefix.len()); ++k)
  {
    ...
    else if (race.kind == Race::OBSERVED && k != j)
    {
      if (!std::any_of(observers.begin(), observers.end(),
                       [this, k](const Event *o) { return o->clock.leq(prefix[k].clock); }))
      {
...
{
  if (iid_map.size() <= unsigned(event.pid))
    iid_map.resize(event.pid + 1, 1);
    ...
    reconstruct_lock_event(const Race &race) const
{
  ...
  assert(std::any_of(prefix[race.first_event].sym.begin(),
                     ...
                     [](const SymEv &e) { return e.kind == SymEv::M_LOCK || e.kind == SymEv::FULLMEM; }));
    ...
    try_find_process_event(IPid pid, int index) const
{
  ...
  assert(prefix[k].iid.get_pid() == pid && prefix[k].iid.get_index() <= index && (prefix[k].iid.get_index() + prefix.branch(k).size) > index);
...
void TSOTraceBuilder::wakeup(Access::Type type, SymAddr ml)
{
  ...
  if (conf.dpor_algorithm != Configuration::SOURCE)
  {
    ...
    for (unsigned p = 0; p < threads.size(); ++p)
    {
      if (!threads[p].sleeping)
        continue;
      assert(threads[p].sleep_sym);
      assert(bool(wakeup_set.count(p)) == do_events_conflict(pid, ev, p, *threads[p].sleep_sym));
    }
  }
#endif
...
long double TSOTraceBuilder::estimate_trace_count(int idx) const
{
  ...
  for (int i = int(prefix.len()) - 1; idx <= i; --i)
  {
    ...
    count += std::max(0, int(prefix.children_after(i))) * (count / (1 + prefix[i].sleep.size()));
  }
