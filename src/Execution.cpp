...
#define IMPLEMENT_POINTER_ICMP(OP)                                                                  \
  case Type::PointerTyID:                                                                           \
    Dest.IntVal = APInt(1, (void *)(intptr_t)Src1.PointerVal OP(void *)(intptr_t) Src2.PointerVal); \
    break;
                                            ...
                                            Type *Ty)
{
  ...
  {
    if (!it->get_ref().overlaps(Src_sas))
      continue;
    ...
    {
      buf[o - Src_sas.addr.offset] = ((char *)it->get_block())[o - it->get_ref().addr.offset];
    }
  }
    ...
    default:
    {
      ...
      { // Other thread
        ...
        { // Stack frame
          ...
          if (EC->CurBB == SF.CurBB)
          {            // Pointing into this basic block
                                  ...
                                  const std::vector<GenericValue> &ArgVals)
{
  ...
  if (tid < 0 || int(Threads.size()) <= tid || tid == CurrentThread)
  {
    ...
    if (tid == CurrentThread)
      ss << " (same as ID of calling thread)";
                                       ...
                                       const std::vector<GenericValue> &ArgVals)
{
  ...
  if (!addr)
    return;
  if (!TB.mutex_init({*addr, 1}))
  { // also acts as a fence
    abort();
    return;
  }
                                       ...
                                       const std::vector<GenericValue> &ArgVals)
{
  ...
  if (ECStack()->size())
  { // Abort did not occur in callPthreadMutexLock above
    GenericValue Result;
                                      ...
                                      const std::vector<GenericValue> &ArgVals)
{
  ...
  if (!cnd_sa || !lck_sa)
    return;
  if (!TB.cond_wait({*cnd_sa, 1}, {*lck_sa, 1}))
  { // also acts as a fence
    abort();
    return;
  }
...
void Interpreter::doPthreadCondAwake(void *cnd, void *lck)
{
  ...
  if (!cnd_sa || !lck_sa)
    return;
  if (!TB.cond_awake({*cnd_sa, 1}, {*lck_sa, 1}))
  { // also acts as a fence
    abort();
    return;
  }
...
{
  int first = 0, len;
  while (first < int(s.size()) && std::isspace(s[first]))
    ++first;
  len = s.size() - first;
  while (0 < len && std::isspace(s[first + len - 1]))
    --len;
...
bool Interpreter::isPthreadMutexLock(Instruction &I, GenericValue **ptr)
{
  ...
  if (!CI)
    return false;
  Function *F = CI.getCalledFunction();
  if (!F || F->getName() != "pthread_mutex_lock")
    return false;
