...
namespace llvm
{
   ...
   */
    ...
    {
      SymMBlockSize(SymMBlock block, uint32_t size) : block(std::move(block)), size(size) {}
      SymMBlock block;
      uint32_t size;
    };
   ...
   */
    ...
    bool isGlobalCmpXchg(Instruction &I);

    void set_rmw_operation(AtomicRMWInst::BinOp op, int val);

    // Opcode Implementations - auxiliary functions (View-equivalence)
    ...
    virtual void visitCallBase(CallBase &CB)
    {
      visitAnyCallInst(&CB);
    }
#else
