...
public:
    ...
    RESULT check_evaluation(std::unordered_map<T, int> valueEnv)
    {
        ...
        for (auto i = objToVal.begin(); i != objToVal.end(); i++)
        {
            auto got = valueEnv.find(i->first);
            if (got != valueEnv.end())
            {
                if (i->second != got->second)
                {
        ...
        assert(sanity());
        llvm::outs() << "PT::checkeval returning " << result_to_string(ret_result) << " for " << to_string() << "\n";
        return ret_result;
    }
    ...
    /* term contains the read event,value pair */
    ...
    void reduce(std::pair<T, int> term)
    {
        for (auto it = objToVal.begin(); it != objToVal.end();) {
            if (it->first == term.first && it->second == term.second) {
                it = objToVal.erase(it);
            }
            else it++;
        }
    }
...
public:
    ...
    void removeTerm(std::pair<T, int> term) {
        ...
        for (auto i = terms.begin(); i != terms.end();) {
            ...
            if (i->in(term)) {llvm::outs() << "removing term " << prev->to_string() << "\n";  prev = terms.erase_after(prev);}
            else {prev = i; i++;}
        }

        if (terms.begin()->in(term)) terms.pop_front();
    }

    void reduceProductTerms(std::pair<T, int> term) {
        for (auto i = terms.begin(); i != terms.end(); i++) {
            if (i->unit()) continue;
            if (!i->in(term)) continue;
            i->reduce(term);
        }
    }
    ...
    friend void operator||(SOPFormula &f1, SOPFormula &f2)
    {
        ...
        SOPFormula f;
        f = f1;
        llvm::outs() << "f = f1 = " << f.to_string() << "\n";
        for (auto j = f2.terms.begin(); j != f2.terms.end(); j++)
        {
            llvm::outs() << "adding term " << j->to_string();
            f1.addTerm(*j);
            llvm::outs() << " makes it " << f1.to_string() << "\n";
        }

        llvm::outs() << "final f1 || f2 = " << f1.to_string() << "\n";
    }
